...
class PursuitGUI(tk.Tk):
    def __init__(self):
        ...
        label = tk.Label(self,text="どちらで遊びますか?")
        label.pack()
    ...
    def make_rlmode(self):
        ...
        #学習方法の選択
        self.RLMethodLabel = tk.Label(self.rl,text="method",width=10,height=1)
        ...
        self.RLMethodLabel2 = tk.Label(self.rl,text="method",width=10,height=1)
        self.RLMethodLabel2.place(x=0,y=55)
        ...
        self.RLMethodCombo2.place(x=80,y=55)

        self.RLMethodLabel3 = tk.Label(self.rl,text="method",width=10,height=1)
        self.RLMethodLabel3.place(x=0,y=55)
        ...
        self.RLMethodCombo3.place(x=80,y=55)

        #学習方法の決定
        self.RLMethodDecisionButton = tk.Button(self.rl,text="OK",
                                               command=lambda:self.decison_method(),
                                               width=5,height=1)
        self.RLMethodDecisionButton.place(x=240,y=52)
        ...
        self.SaveQLabel.place(x=0,y=191)
        ...
        self.SaveQTrue = tk.Button(self.rl,text="True",bg="white",
                                     ...
                                     width=6,height=1)
        self.SaveQTrue.place(x=80,y=191)
        self.SaveQFalse = tk.Button(self.rl,text="False",bg="white",
                                     ...
                                     width=6,height=1)
        self.SaveQFalse.place(x=160,y=191)

        #学習の実行
        self.TrainStartButton = tk.Button(self.rl,text="START",
                                    command=lambda: self.train_start(),
                                    width=7,height=1)
        self.TrainStartButton.place(x=230,y=218)
    
    def decison_method(self):
        pass
    ...
    def train_start(self):
        ...
        if self.RLMethodCombo.get() == "Q-Learning":
            ...
            #messagebox.showinfo("エラー","未実装です。")
            path_name = str(self.OtherMethodText.get('1.0','end -1c'))
            class_name = str(self.MethodClassText.get('1.0','end -1c'))

            try:
                #文字列からimport先を指定
                module = import_module(path_name)
                #オブジェクトの生成
                for i  in range(int(self.AgentsNumCombo.get())):
                    temp = module.globals()[class_name](aid=i,actioins=self.action,training=True)
                    self.agents.append(temp)
            except:
                messagebox.showinfo("エラー","インポートできませんでした")
                return 0
    ...
    def trainthread(self,graph_flag,map_size,train_num,non_vision,save_flag):
        ...
        #グラフ化
        if graph_flag == 1:
            self.result = pd.Series(self.result).rolling(50).mean().tolist()
            plt.plot(np.arange(len(self.result)),self.result)
            plt.xlabel("Episodes")
            plt.ylabel("Steps")
            plt.legend()
            plt.show()
